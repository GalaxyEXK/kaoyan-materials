...
注意：其中 $\left\{ \begin{array}{l} x = x(u, v), \\ y = y(u, v) \end{array} \right.$ 是 $(x, y)$ 面到 $(u, v)$ 面的一对一映射， $x = x(u, v), y = y(u, v)$ 存在一阶连
...
(5) $D = \{(x,y)\mid 1\leqslant x^2 +y^2\leqslant \mathrm{e}^2,x\geqslant 0,y\geqslant 0\}$
...
(6) $D = \left\{(x,y)\mid 0 <   y <   \sqrt{2ax - x^2},x > y,a > 0\right\} .$ (7） $D = \{(x,y)\mid 0\leqslant x\leqslant 2t,0\leqslant y\leqslant t,t > 0\}$
...
方法总结 $\int_{c}^{d}\mathrm{d}y\int_{x_1(y)}^{x_2(y)}f(x)\mathrm{d}x$ 对 $x$ 积分比较困难时，可转化为先积 $y$ ，即 $\int_a^b\mathrm{d}x\int_{y_1(x)}^{y_2(x)}f(x)\mathrm{d}y.$
