...
$\left\{ \begin{array}{l} \mathrm{e}^{\alpha x}\text{照抄},\\ Q_{n}(x)\text{为} x\text{的} n\text{次多项式},\\ k = \left\{ \begin{array}{ll}0, & \alpha \text{不是特征根},\\ 1, & \alpha \text{是单特征根},\\ 2, & \alpha \text{是二重特征根}. \end{array} \right. \end{array} \right.$
...
设 $y^{*}=\mathrm{e}^{x}(A\cos2x+B\sin2x)x$ 一看： $\alpha\pm\beta i=1\pm2i$ 二算： $r_{1,2}=1\pm2i$
...
$$
\begin{array}{l} {\mathrm{(3)} \frac {1}{F (\mathrm{D})} (x ^ {k} + a _ {1} x ^ {k - 1} + \dots + a _ {k - 1} x + a _ {k}) \text {型}.} \\ {y ^ {*} = \frac {1}{F (\mathrm{D})} (x ^ {k} + a _ {1} x ^ {k - 1} + \dots + a _ {k - 1} x + a _ {k}) = Q _ {k} (\mathrm{D}) (x ^ {k} + a _ {1} x ^ {k - 1} + \dots + a _ {k - 1} x + a _ {k}).} \\ {\mathrm{常借助} \frac {1}{1 - x} \mathrm{的} k \mathrm{次泰勒多项式} 1 + x + x ^ {2} + \dots + x ^ {k}} \\ {\mathrm{二者对应}.} \end{array}
$$
...
原方程的通解为 $y = C_1\mathrm{e}^x +C_2\mathrm{e}^{2x} + xe^x$ ，其中 $C_1,C_2$ 为任意常数.
