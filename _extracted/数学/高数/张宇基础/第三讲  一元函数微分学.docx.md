...
点点相依相偎的图 3-5(a)\~(c)，加上绝对值后依然相依相偎成为图 3-6(a)\~(c)，故成立（无论是还是，只要相依相偎即可）。为什么反过来不对？很简单，你看 $|f(x)|$ 相依相偎，连续 [见图 3-7(b)]，可 $f(x)$ 却相距甚远，自然不连续 [见图 3-7(a)]。
...
3.7 证明 (1) $F_{+}^{\prime}(x_0) = \lim_{x\to x_0^{+}}\frac{F(x) - F(x_0)}{x - x_0}\xlongequal{\text{洛必达法则}}\lim_{x\to x_0^{+}}\frac{F'(x)}{1} = A$ .
