...
例2.13 设正项数列 $\{a_{n}\}$ 单调增加, 则以下选项中使得 $\{a_{n}\}$ 收敛的是( ). (A) $\left\{(1 + a_n)^{\frac{1}{a_n}}\right\}$ 收敛于1 (B) $\left\{\left(1 + \frac{1}{a_n}\right)^{a_n}\right\}$ 收敛于e (C) $\{a_{n}\ln a_{n}\}$ 收敛于0 (D) $\left\{\frac{\ln a_n}{a_n}\right\}$ 收敛于0
...
当 $a_{n}\ln a_{n}\rightarrow0$ 时， $a_{n}\rightarrow0^{+}$ 或 $a_{n}\rightarrow1$ ，又 $a_{n}>0$ 且单调增加，所以 $a_{n}\rightarrow1^{-}$ ，故 $\{a_{n}\}$ 收敛.
对于选项(D)， $\frac{\ln x}{x}(x>0)$ 的图像如图(d)所示.
