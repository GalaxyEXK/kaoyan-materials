...
向空树 $T = (U, E_T)$ 中添加图 $G = (V, E)$ 中的任意一个顶点 $u_0$ ，完成初始集合的赋值：
...
循环（重复下列操作直至 U=V）：从图 G 中选择满足 $\{(u,v)\mid u\in U, v\in V-U\}$ 且具有最小权值的边 $(u,v)$ ，加入树 T，置 $U=U\cup\{v\}$ ， $E_{T}=E_{T}\cup\{(u,v)\}$ 。
Prim 算法的简单实现如下：
...
检查当前欠掉自 $V_i$ 的顶点，若其 final 值为 false 则更新 dist 和 path 信息
...
<table><tr><td>对比项</td><td>BFS 算法</td><td>Dijkstra 算法</td><td>Floyd 算法</td></tr><tr><td>用途</td><td>求单源最短路径</td><td>求单源最短路径</td><td>求各顶点之间的最短路径</td></tr><tr><td>无权图</td><td>适用</td><td>适用</td><td>适用</td></tr><tr><td>带权图</td><td>不适用</td><td>适用</td><td>适用</td></tr><tr><td>带负权值的图</td><td>不适用</td><td>不适用</td><td>适用</td></tr><tr><td>带负权回路的图</td><td>不适用</td><td>不适用</td><td>不适用</td></tr><tr><td>时间复杂度</td><td> $O(V^2)$  或  $O(V+E)$ </td><td> $O(V^2)$ </td><td> $O(V^3)$ </td></tr></table>
